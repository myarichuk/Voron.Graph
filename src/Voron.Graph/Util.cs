using System;
using System.Collections.Generic;
using System.Linq;
using System.Threading.Tasks;

namespace Voron.Graph
{
	public static class Util
	{
		//taken from here
		//http://jasonf-blog.blogspot.co.il/2006/08/hanselmans-endianess-converter.html
		private static readonly byte[] _flippedBytes = {
			
		};

		private static readonly ushort[] _flippedWords = new ushort[1 << 16];

		static Util()
		{
			for (int i = 0; i < _flippedWords.Length; i++)
			{
				_flippedWords[i] = (ushort)((_flippedBytes[i & 0xff] << 8)
										   | _flippedBytes[(i >> 8)]);
			}
		}

		public static long ReverseBits(long num)
		{
			long z = 0;
			const int bits = sizeof(long) * 8;
			z = (z << 16) | _flippedWords[num & 0xffff];
			num = num >> 16;
			z = (z << 16) | _flippedWords[num & 0xffff];
			num = num >> 16;
			z = (z << 16) | _flippedWords[num & 0xffff];
			num = num >> 16;
			z = (z << 16) | _flippedWords[num & 0xffff];
			num = num >> 16;

			return z >> (64 - bits);
		}
	}
}
