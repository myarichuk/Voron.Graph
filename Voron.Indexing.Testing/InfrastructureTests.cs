...
namespace Voron.Indexing.Testing
{
	public class InfrastructureTests : IDisposable
	{
		...
		{
			currentFolder = new FileInfo(GetType().Assembly.Location).Directory.FullName;
			dataDir = currentFolder + Path.DirectorySeparatorChar + "Data";
        }

		[Fact(DisplayName = "Stopword filter should remove all stopwords")]
		...
		[Fact(DisplayName = "Should persist stored field names")]
		public void Should_persist_stored_field_names()
		{
			...
			{
				var index = new Index("TestIndex", env, new NGramTokenizer());
				index.IndexedFields.Should().Contain("foo", "bar", "baa");
			}
		}
